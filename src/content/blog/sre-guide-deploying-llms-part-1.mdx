...
To ground this, take the sentence: "My cat doesn't feel like going outside because it's raining." In the sentence, we know that "it's" is referring to "raining", not the "cat". That shows up directly in the numbers: the query from "it's" dotted with the key from "raining" scores higher than the same query dotted with the key from "cat". So when the weighted values are summed, "raining" contributes the most, and "it's" comes out carrying the sense that this is about the weather, worked out purely from the surrounding tokens.
